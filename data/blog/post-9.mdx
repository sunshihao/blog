...
1. 做一件事，并做好它：每个程序应该专注于完成一个特定的任务，并且做到最好[1](https://lackofimagination.org/2024/01/on-unix-philosophy/)。这种专注使得程序更容易理解、维护和改进。

2. 程序之间的协作：设计程序时应考虑到它们可以与其他程序协同工作[1](https://lackofimagination.org/2024/01/on-unix-philosophy/)。这种方法鼓励创建可以轻松组合在一起的小型、专门化的工具。

3. 文本流作为通用接口：使用文本流作为程序之间交换数据的主要方式[1](https://lackofimagination.org/2024/01/on-unix-philosophy/)。文本是一种通用的、人类可读的格式，可以被各种工具处理和操作。

4. 模块化设计：鼓励将复杂的系统分解成更小、更易管理的部分。这使得系统更容易理解、维护和扩展。

5. 组合而非扩展：当需要新功能时，优先考虑创建新的模块并将其与现有模块组合，而不是通过添加新特性来使现有程序变得复杂[3](https://www.tedinski.com/2018/05/08/case-study-unix-philosophy.html)。
...
10. 数据流处理：设计能够处理数据流的程序，因为这是一种通用的接口[1](https://lackofimagination.org/2024/01/on-unix-philosophy/)。
